...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (campaign.user_id !== userId) {
      return NextResponse.json(errorResponse("Tidak diizinkan menghapus campaign ini", 403), { status: 403 });
    }
    ...
    return NextResponse.json(successResponse("Campaign berhasil dihapus", null));
  } catch (error) {
